...
def installment_features(data, group_feature):
    ...
    prev_len['cnt'] -= 1
    prev_len = data.groupby([group_feature]).size() - prev_len.groupby([group_feature])['cnt'].sum()
    prev_len = prev_len.reset_index().rename(columns={0: 'cnt'})
    prev_len = prev_len[[group_feature, 'cnt']].set_index(group_feature)
    
    features = []
    real_delay_cnt = data.groupby(group_feature)['is_delay'].sum()
    real_delay_ratio = real_delay_cnt / data.groupby(group_feature).size()
    fake_delay_cnt = single_data.groupby(group_feature)['is_delay'].sum() 
    fake_delay_ratio = fake_delay_cnt / single_data.groupby(group_feature).size()
    features += [real_delay_cnt, real_delay_ratio, fake_delay_cnt, fake_delay_ratio]
    
    prompt_pay = data[~data['is_delay']].groupby(group_feature)['delay_delta'].agg(['min', 'max', 'mean', 'median', 'std'])
    late_pay = single_data[single_data['is_delay']].groupby(group_feature)['delay_delta'].agg(['min', 'max', 'mean', 'median', 'std'])
    features += [prompt_pay, late_pay]
    
    data.loc[data['delay_days']==0, 'last_day_pay'] = 1
    last_day_pay_cnt = data.groupby(group_feature)['last_day_pay'].sum().to_frame()
    ...
    rep_cnt = rep_data.groupby(group_feature)['rep'].sum()
    rep_ratio = rep_cnt / rep_data.groupby(group_feature).size()
...
if __name__ == '__main__':
    ...
    x_0 = installment_features(instal[instal['DAYS_INSTALMENT']>=-60], 'SK_ID_CURR')
    x_1 = installment_features(instal[instal['DAYS_INSTALMENT']>=-180], 'SK_ID_CURR')
    x_2 = installment_features(instal[instal['DAYS_INSTALMENT']>=-365], 'SK_ID_CURR')
    x_3 = installment_features(instal, 'SK_ID_CURR')
    
    fixed_features = ['SK_ID_CURR', 'SK_ID_PREV', 'NAME_CONTRACT_STATUS']
    X = installment_features(instal, 'SK_ID_PREV')
    entry_instal_diff = instal.groupby('SK_ID_PREV')['diff_entry_instal'].agg(['min', 'max', 'mean', 'median', 'std']).add_prefix('entry_instal_diff_')
    duration = instal.groupby('SK_ID_PREV')['DAYS_INSTALMENT'].max() - instal.groupby('SK_ID_PREV')['DAYS_INSTALMENT'].min()
    X = pd.concat([X, entry_instal_diff, duration], axis=1)
